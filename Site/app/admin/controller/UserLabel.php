...
use app\common\logic\admin\UserLabel as UserLabelLogic;
...
class UserLabel extends Base
{
    ...
    {
        return $this->fetch('/user_label_add');
    }
    ...
    {
        $logic = new UserLabelLogic();
        $data = $logic->readData($id);
        if (!empty($data)) {
            $this->assign('data', $data);
            return $this->fetch('/user_label_edit');
        }
        else {
    ...
    public function save() : Json
    {
        if (Request::isPost() && Request::isAjax()) {
            ...
            $result = $validate->scene('label')->check($param);
            if (!$result) {
