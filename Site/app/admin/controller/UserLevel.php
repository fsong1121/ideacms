...
use app\common\logic\admin\UserLevel as UserLevelLogic;
...
class UserLevel extends Base
{
    ...
    {
        return $this->fetch('/user_level_add');
    }
    ...
    {
        $logic = new UserLevelLogic();
        $data = $logic->readData($id);
        if (!empty($data)) {
            $this->assign('data', $data);
            return $this->fetch('/user_level_edit');
        }
        else {
    ...
    public function save() : Json
    {
        if (Request::isPost() && Request::isAjax()) {
            ...
            $result = $validate->scene('level')->check($param);
            if (!$result) {
