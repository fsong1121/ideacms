...
class MobileMenu extends Base
{
    ...
    {
        return MobileMenuModel::find($id);
    }
    ...
    {
        try {
            $list = MobileMenuModel::where('id', '>', '0');
            if($param['keys'] != '') {
    ...
    {
        $list = new MobileMenuModel();
        if(isset($param['m_id'])) {
            $list = MobileMenuModel::find($param['m_id']);
        }
        try {
            $data = [
                'title' => $param['m_title'],
                'url' => $param['m_url'],
                'url_type' => $param['m_url_type'],
                'pic' => $param['m_pic'],
                'info' => $param['m_info'],
                'type' => $param['m_type'],
                'sequence' => $param['m_px'],
