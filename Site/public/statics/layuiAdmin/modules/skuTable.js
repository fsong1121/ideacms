...
layui.define(['jquery', 'form', 'upload', 'layer', 'sortable'], function (exports) {
    ...
    class SkuTable {
        ...
        options = {
            ...
            singleSkuTableConfig: {
                thead: [
                    ...
                    {title: '划线价(元)', icon: 'layui-icon-cols'},
                ...
                tbody: [
                    ...
                    {type: 'input', field: 'stock', value: '99', verify: 'required|number', reqtext: '库存不能为空'},
        ...
        listen() {
            ...
            $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-create`, function () {
                ...
                layer.prompt({title: '规格'}, function (value, index, elem) {
                    Util.msg.close(index);
                    that.data.specData.forEach((item, index) => {
                        if (item.title == value) {
                            flag = false;
                            Util.msg.tips('规格已存在');
                        }
                        ...
                        Util.request.post(
                            {
                                ...
                                data: {id: specId, title: value, product_type_id: that.data.productTypeId}
                            },
                            function (res) {
                                $('#spec_id').val(parseInt(specId) + 1);
            ...
            $(document).on('click', `#${this.options.specTableElemId} .fairy-spec-value-create`, function () {
                ...
                var specItemId = $('#spec_item_id').val();
                        ...
                        Util.request.post(
                            {
                                ...
                                data: {id: specItemId, spec_id: specId, title: value}
                            },
                            function (res) {
                                $('#spec_item_id').val(parseInt(specItemId) + 1);
                                that.data.specData.forEach(function (v, i) {
                                    if (v.id == specId) {
        ...
        renderFormItem(label, content, target) {
            ...
            if(label == '规格模板') {
                html += '<div class="layui-input-inline w180">';
            } else {
                if(label == '商品规格') {
                    html += '<div class="layui-input-block spec_ctr">';
                } else {
            ...
            html += '</div>';
            if(label == '规格模板') {
                html += '<div class="layui-form-mid layui-word-aux">可选择规格模板，也可自行添加</div>';
            }
            html += '</div>';
        ...
        renderProductType() {
            Util.request.get({url: this.options.productTypeUrl}, (res) => {
                ...
                html += '</select>';
                this.renderFormItem('规格模板', html, this.options.productTypeElemId);
                if (this.data.productTypeId) {
                    ...
                    if(this.data.productId) {
                        //编辑默认规格
                        Util.request.get({url: this.options.editSpecUrl, data: {goods_id: this.data.productId}}, (res) => {
                            this.data.specData = res.data.spec;
                            this.renderSpecTable();
                            this.renderMultipleSkuTable();
                        });
        ...
        renderSpecTable() {
            ...
            $.each(this.data.specData, function (index, item) {
                ...
                $.each(item.options, function (key, value) {
                    table += `<input type="checkbox" lay-skin="primary" title="${value.title}" lay-filter="fairy-spec-filter" value="${value.id}" ${item.value.includes(value.id) ? 'checked' : ''} /> <i class="layui-icon layui-icon-delete layui-anim layui-anim-scale ${that.data.specDataDelete ? '' : 'layui-hide'}" data-spec-value-id="${value.id}"></i> `;
                });
        ...
        renderMultipleSkuTable() {
            ...
            if ($(`#${this.options.specTableElemId} tbody input[type=checkbox]:checked`).length) {
                ...
                table += '<colgroup>' + '<col width="90">'.repeat(prependThead.length + 1) + '</colgroup>';
                ...
                prependTbody.reduce(function (prev, cur, index, array) {
                    ...
                    that.options.multipleSkuTableConfig.tbody.forEach(function (c) {
                        switch (c.type) {
                            ...
                            case "input":
                            default:
                                tr += '<td><input type="text" name="' + c.field + '[]" value="' + (that.data.skuData[that.makeSkuName(item, c)] ? that.data.skuData[that.makeSkuName(item, c)] : c.value) + '" class="layui-input" lay-verify="' + c.verify + '" lay-reqtext="' + c.reqtext + '"></td>';
                                break;
                        }
                    });
                    tr += '<input type="hidden" name="specKey[]" value="'+item.title+'">';
                    tr += '<input type="hidden" name="specName[]" value="'+that.makeSpecName(item.title)+'">';
                    tr += '</tr>';
            ...
            this.renderFormItem('规格详情', table, this.options.skuTableElemId);
        ...
        makeSkuName(sku, conf) {
            return '[' + (this.options.skuNameType === 0 ? sku.id : sku.title) + '][' + conf.field + ']';
        }
        ...
        makeSpecName(sku) {
            ...
            skuArr.forEach((item, index) => {
                str = str + this.data.specData[index].title.trim() + ":" + item + " ";
            });
            return str;
        }
        ...
        getSpec() {
            ...
            this.data.specData.forEach((item, index) => {
                ...
                item.options.forEach((v, k) => {
                    var isSelect = 0;
                    if(item.value.includes(v.id)) {
                        isSelect = 1;
                    }
                    options.push({title:v.title,select:isSelect});
                });
                specList.push({spec:item.title,items:options});
            });
            return JSON.stringify(specList);
        }
    }
